...
export class MessageBusAdapter implements IMessageBusAdapter {
  ...
  private async getInstance(options: GetInstanceOptions): Promise<MessageBus> {
    if (this._messageBus === null) {
      switch (this._config?.messageBusType) {
        case MessageBusType.withConfirmation:
          this._messageBus = await amqplib.getConfirmChannel(options.vHost);
          break;
        case MessageBusType.noConfirmation:
        default:
          this._messageBus = await amqplib.getChannel(options.vHost);
          break;
      }
    }
  ...
  async publish(
    ...
    const msgBusInstance = await this.contentToBuffer
